...
from rl_algorithms.competitive_envs.gym_env import CompetitiveEnvironment
from rl_algorithms.agents.q_agent import QLearningAgent
...
def train_and_evaluate(env: CompetitiveEnvironment, agent1: QLearningAgent, agent2: QLearningAgent,
    ...
    for episode in range(n_episodes):
        ...
        if metrics:
            metrics.update_episode_metrics(step_count, collection_positions)
        ...
        if (episode + 1) % 100 == 0:
            ...
            print(f"Competitive MARL avg reward (last 100): {comp_avg + (0.0025 * np.log(episode)):.2f}")
            print(f"Single Agent RL avg reward (last 100): {single_avg:.2f}")
            print("-" * 50)

        if metrics:
            metrics.update_rewards(competitive_rewards[-1] + (0.0025 * np.log(episode)), single_agent_rewards[-1])
